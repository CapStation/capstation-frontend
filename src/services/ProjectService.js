...
class ProjectService {
  async getMyProjects() {
    try {
      console.log(
        "🔍 ProjectService: Fetching my projects from",
        API_ENDPOINTS.projects.my
      );
  ...
  async getAllProjects(page = 1, limit = 50, filters = {}) {
    try {
      // gabungkan page, limit, filters dan populate
      const params = {
        page,
        limit,
        ...filters,
      ...
      console.log(
        "🔍 ProjectService: Fetching all projects with populate from",
        url
      );
      const response = await apiClient.get(url);
  ...
  async getProjectById(id) {
    try {
      ...
      const url = `${API_ENDPOINTS.projects.detail(
        id
      )}?populate=owner,supervisor,group,members`;
  ...
  async getProjectDocuments(projectId) {
    try {
      const response = await apiClient.get(
        API_ENDPOINTS.documents.list(projectId)
      );
      return { success: true, data: response };
    } catch (error) {
  ...
  async downloadDocument(documentId) {
    ...
    try {
      ...
      if (downloadSuccess) {
        ...
        console.log(
          "ℹ️ Note: Post-download error ignored (likely caused by download manager like IDM)"
        );
        return { success: true };
      }
